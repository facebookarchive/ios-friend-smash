...
namespace FriendSmasher
{
    namespace Game
    {
            ...
            [req startWithCompletionHandler:^(FBRequestConnection *connection, id result, NSError *error)
             {
                 if (result && !error)
                 {
                     ...
                     NSArray* fetchedPermissionData = [[NSArray alloc] initWithArray:[result objectForKey:@"data"]];
                     
                     bool bFound = false;
                     
                     for (NSDictionary *currper in fetchedPermissionData) {
                         if( [[currper valueForKey:@"permission"] caseInsensitiveCompare:permission] == NSOrderedSame ) {
        ...
        {
            FBRequest *req = [[FBRequest alloc] initWithSession:[FBSession activeSession] graphPath:@"me/invitable_friends"];
